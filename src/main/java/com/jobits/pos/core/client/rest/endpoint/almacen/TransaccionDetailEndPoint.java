 ...
 */
@RestController
@RequestMapping(path = "/transaccion_detail")
public class TransaccionDetailEndPoint extends CrudRestServiceTemplate<Transaccion> {

    TransaccionModelAssembler transaccionAssembler = new TransaccionModelAssembler();

    @Override
    public CRUDUseCase<Transaccion> getUc() {
        return PosCoreModule.getInstance().getImplementation(TransaccionDetailService.class);
    }
